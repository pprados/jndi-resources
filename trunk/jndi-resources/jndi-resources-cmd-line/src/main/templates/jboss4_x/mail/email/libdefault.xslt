...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="basefilename">
		<xsl:value-of
			select="concat('deploy/',$currentid,'/',$currentid,'_email-service')" />
	</xsl:variable>
	...
	<xsl:template
		...
		>
			<xsl:if test="not(jndi:property[@name='email'])">
				<xsl:value-of
					select="error(sch:QName('error9999'),concat('The property email must be set for resource ',@familly))" />
			</xsl:if>
			...
			<attribute name="Factory">org.jndiresources.factory.InternetAddressFactory</attribute>
			<attribute name="Configuration">
				<configuration>
					<property name="value"
						value="{jndi:property[@name='email']/@value}" />
				</configuration>
			</attribute>
