...
<xsl:template match="/">
	<xsl:if test="$war != ''">
		<xsl:value-of select="tools:mkLink(concat($parentwar,$war),concat($targetdir,'../',$war),concat($targetdir,'jboss.server.conf/deploy/',$currentid,'/',$war))"/>
	</xsl:if>
	<xsl:value-of select="tools:fileCopyIfExist('templates.properties',concat($targetdir,'templates.properties'))"/>
	<xsl:value-of select="tools:fileCopyIfExist('templates.xpath',concat($targetdir,'templates.xpath'))"/>
</xsl:template>
