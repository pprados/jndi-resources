...
<xsl:variable  name="basefilename"><xsl:value-of select="concat('deploy/',$currentid,'/',$currentid,'_jcli_mail-service')"/></xsl:variable>
...
<xsl:template match="jndi:resources[@id=$currentid]/jndi:resource[@familly=$familly]">
		<xsl:text>&#xA;   </xsl:text>
		<xsl:comment>Register link to Javamil for key <xsl:value-of select="@name"/></xsl:comment>
		<xsl:text>&#xA;   </xsl:text>
