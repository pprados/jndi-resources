...
<xsl:variable  name="basefilename">
	<xsl:value-of select="concat('catalina.base/conf/',$currentid,'_hosts-service')"/>
</xsl:variable>
...
<xsl:template match="jndi:resources[@id=$currentid]/jndi:resource[@familly=$familly]">
		...
		<Resource auth="Container" type="java.net.InetAddress">
		...
		<ResourceParams>
			...
			<parameter>
				<name>factory</name>
				<value>com.googlecode.jndiresources.factory.InetAddressFactory</value>
			</parameter>
	        ...
	        <xsl:if test="not(jndi:property[@name='host'])">
				<xsl:value-of select="error(sch:QName('error9999'),concat('The property host must be set for resource ',@familly))"/>
	        </xsl:if>
			<parameter>
				<name>value</name>
				<value><xsl:value-of select="jndi:property[@name='host']/@value"/></value>
			</parameter>
		</ResourceParams>
      
	 	<xsl:call-template name="install-drivers">
			<xsl:with-param name="familly" select="'host/default'"/>
		</xsl:call-template>
 </xsl:template>
