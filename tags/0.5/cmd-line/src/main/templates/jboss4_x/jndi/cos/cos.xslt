...
<xsl:variable  name="basefilename"><xsl:value-of select="concat('deploy/',$currentid,'/',$currentid,'_jndi_cos-service')"/></xsl:variable>
<xsl:variable  name="filename"><xsl:value-of select="concat($targetdir,'jboss.server.conf/',$basefilename,'.jndi')"/></xsl:variable>

<xsl:variable name="jndi.driver.jar"><xsl:value-of select="$drivers/familly[@name=$familly]/@value"/></xsl:variable>
<xsl:variable name="InitialContext">javax.naming.InitialContext</xsl:variable>
<xsl:variable name="Properties">
java.naming.factory.initial=com.sun.jndi.cosnaming.CNCtxFactory
java.naming.provider.url=${jndi.cos.url}
java.naming.batchsize=${jnbi.cos.batchsize}
</xsl:variable>
