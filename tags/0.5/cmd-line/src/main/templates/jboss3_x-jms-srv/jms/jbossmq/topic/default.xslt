...
<xsl:variable  name="basefilename"><xsl:value-of select="concat('deploy/',$currentid,'/',$currentid,'_srv_jbossmq_topic-service')"/></xsl:variable>
...
<xsl:template match="jndi:resources[@id=$currentid]/jndi:resource[@familly=$familly]">
	...
	>
		...
		<xsl:if test="./jndi:property[@name='dead-letter-queue']">
			<xsl:value-of select="tools:warn(concat('It''s impossible to declare a specific dead letter queue for ',$appsrv,'/',$familly))"/>
		</xsl:if>
