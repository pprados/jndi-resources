...
<xsl:variable name="basefilename"><xsl:value-of select="concat($currentid,'_cf-service')"/></xsl:variable>
<xsl:variable name="filename"><xsl:value-of select="concat($targetdir,'catalina.base/conf/',$basefilename,'.jndi')"/></xsl:variable>
<xsl:variable name="type">javax.jms.ConnectionFactory</xsl:variable>
<xsl:variable name="driver">jms/jbossmq</xsl:variable>
